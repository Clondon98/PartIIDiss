...
During the course of this model I ran the models on three different datasets.
...
Evaluating the performance of a machine learning algorithm should be done on a test dataset that is seperate from the dataset the 
algorithm is trained on, to detect overfitting. If the dataset is not particularly large this can be done using 
...
Therefore I instead partitioned my test dataset in two, a validation set and a test set. The hyperparameters are optimised using the 
...
While the cross-validation changes made above reduce the number of iterations to 
