...
algorithm is trained on. If the dataset is not particularly large this can be done using \textbf{k-fold cross validation}, where the 
data is split into k different partitions. k-1 of these partitions are used for training, while the kth is used to test the performance 
after training. The accuracy for the model is then computed by taking the average of all the accuracies for each fold.
