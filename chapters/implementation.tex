...
The models used in this project all need the same methods to allow the scripts used for running and evaluation to be as 
...
\texttt{nan} as the variance of the latent distribution becomes infintesimal.
...
for a fair comparison. It also allows each script to run hyperparameter optimisation over only one fold at a time. The Cambridge High Performance
