...
Autoencoder architectures are all based on \textbf{neural networks} so it is important to have a good understanding of the basic principles
of these networks.
...
The main activation functions used in this project is ReLU, which has the form:
\begin{align}
  \sigma(z) = max(0, z)
  \label{eq:relu}
\end{align}
It is sed in the hidden layers of multilayer perceptrons to provide non-linearity, allowing the network to learn more 
complicated functions. It has seen massive uptake due to performance benefits over the previously popular sigmoid function~\cite{relu}.
...
topological space that is locally Euclidea'', i.e. there is a neighbourhood around each point on the manifold that can be describe as n-dimensional 
...
i.e. it can be described using fewer features. Again this can be seen intuitively by the fact that the set of 784 pixel images 
that look like a recognisable digit is a very small subset of the set of 784 pixel images - randomly selected pixel values look like 
random noise, suggesting that there is a lot of redundancy in the MNIST features.
\begin{figure}[H]
  \centering
  \begin{subfigure}[b]{0.4\linewidth}
    \centering
    \includegraphics[scale=4]{figs/rand_noise.png}
    \caption{Pixel values selected randomly}
  \end{subfigure}
  \hspace{0.1\textwidth}
  \begin{subfigure}[b]{0.4\linewidth}
    \centering
    \includegraphics[scale=4]{figs/digit.png}
    \caption{MNIST digit}
  \end{subfigure}
...
\begin{figure}[H]
  \begin{center}
      \scalebox{.6}{\input{figs/autoencoder.tex}}
...
Autoencoders can be trained end to end using backpropagation as explained in section \ref{backprop}. The target $\vec{x}$ and output 
$\vec{\hat{x}}$ are both vectors and so 
the squared \textbf{Euclidean distance}\footnote{also known as mean square error} is used as the loss per datapoint. Both the encoder weights 
$\vec{\theta}$ and 
...
\footnote{when sigmoid was used as the activation function the gradient in each layer would usually be less than one, and so the multiplication 
of these gradients in backpropagation would lead to very small gradients in the lower layers}
...
representation $\vec{z}$. The latent distribution describes the variation in the data, and so these are referred to as 
...
~\cite{Tutorial70:online}.

The decoder of the VAE is a neural network that models the likelihood, $p_{\vec{\phi}}(\vec{x}|\vec{z})$. Once the VAE is trained new samples similar to 
...
This is the encoder, and it learns to approximate the posterior by maximising the evidence.
...
of this loss can be found in Appendix~\ref{vae_loss}.
\begingroup
\allowdisplaybreaks
\begin{align}
...
reconstruction loss (squared Euclidean distance) between the input and output, and the \textbf{Kullback-Leibler divergence} between 
...
trained as part of a one layer denoising autoencoder, with layer to be trained used as the encoder and a new temporary layer constructed
...
\begin{figure}[H]
  ...
  \begin{subfigure}[b]{0.4\linewidth}
    \centering
    \scalebox{.75}{\input{figs/generative.tex}}
    \caption{The generative model}
  \end{subfigure}
  \begin{subfigure}[b]{0.4\linewidth}
    \centering
    \scalebox{.75}{\input{figs/inference.tex}}
...
In the semi-supervised model there are two inference cases. When the data is labelled the question is of how to infer $\vec{z}$ from $\vec{x}$ and $y$.
Both $\vec{x}$ and $\vec{y}$ are used as input to both the encoder, while $y$ and $\vec{z}$ are used as input to the decoder.
This helps the encoder to learn to separate the representation of $y$ and $z$, so that $z$ contains
...
the model should 
maximise the evidence $p(\vec{x}, y)$, the probability the model assigns to the real data. This leads to a variant of the ELBO by marginalizing
out $z$:
\begin{align}
...
by a neural network, and outputs a categorical distribution over the labels. The summation term 
...
Currently at no point does the classifier learn directly from the labelled data, unnecessarily disadvantaging the model. To remedy
...
looked at so far in that they attempt to represent the data using multiple layers of latent variables, rather than a 
...
more abstract, 
as they don't have to model the lower level details encoded in the lower layers. 
...
further into the network modelling more complicated and abstract features with the final layer just outputting a class label.
...
a machine learning model. To this end I decided upon a list of essential reading:~\cite{ML_Bayes}~\cite{Goodfellow-et-al-2016}
...
I made the decision to use Python, as it has support for the deep learning library PyTorch.  After trying both PyTorch and Tensorflow, 
